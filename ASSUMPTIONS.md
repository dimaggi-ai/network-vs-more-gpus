...
| A18 | Rank placement across a stitch: contiguous blocks (`hierarchical`) or interleaved (`blind`) | hierarchical | both reported everywhere | Not an assumption so much as the study's independent variable. A6 assumes dense regular placement *within* a hall and calls it the best case for the network; A18 is the same question across halls, and the answer is that it dominates distance. Every atlas cell is reported under both. |
| A19 | A stitch is a bandwidth and a latency, nothing else | as stated | not swept | No insertion loss, bit-error rate, circuit flap, or retune. Whether the declared circuit is the measured circuit is a separate world-model problem this repository does not address, and every claim here assumes the circuit delivers what it advertises. Listed in the registry's DECLINED output. |
| A20 | The span circuit is an aggregate shared by every group crossing it, divided evenly, and each rank's share is capped at its own NIC line rate | shared, NIC-capped | width swept 100 Gbit/s to 204.8 Tbit/s | The tier differs from the other three in being a circuit rather than an entitlement, which is why contention scales with the cluster. Removing the NIC cap makes an unbounded circuit outrun the fabric the ranks sit on; `tests/test_span.py` deletes it and requires the registry to go red. |
| A21 | Two halls | 2 | not swept | The ring term is general and `halls > 2` is accepted, but no published measurement was found to check a three-hall configuration against, so no result here is reported for one. DECLINED in the registry. |
| A22 | Synchronous training only | synchronous | not swept | DiLoCo-class low-communication methods (S17) change the communication structure the atlas assumes. No cell here describes them. |
