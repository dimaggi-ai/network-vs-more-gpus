...
rising to 4.6 at 65{,}536, because a marginal accelerator is not fully
productive. Third, no single intervention
dominates: across the regimes we examine, three different interventions rank
first, and under parameter uncertainty at 16{,}384 accelerators additional
bandwidth ranks first in 0.6\% of draws while reliability and recovery
improvements account for the other 99.4\%. The title's
...
share only the parameter definitions. Their agreement is a
validation requirement, and a recovery-pressure scope guard bounds the regime
the model is applied to (Section~\ref{sec:validation}).
...
We define \emph{recovery pressure} as the share of runtime consumed by
discarded work plus restart, and we exclude configurations above 0.25 from
headline claims. The exclusion is a modeling-scope judgment, not a fidelity
boundary: the two implementations agree to within 0.02\% on UCF at every tested
severity, including far beyond the threshold (Figure~\ref{fig:fidelity}).
Above it, a job would be spending more than a quarter of its life on discarded
work and restart, which is outside the regime the model's independence
assumptions were calibrated for and not a state any operator would leave
uncorrected. An earlier version of the event-driven path mis-attributed
detection time and appeared to diverge at high severity; an independent code
review found the error, and the correction is recorded in the repository's
decision log (D14) rather than silently absorbed.

\begin{figure}[t]
\centering
\includegraphics[width=0.62\textwidth]{../figures/fig7_cross_fidelity.pdf}
\caption{Cross-fidelity agreement. The two independent implementations agree
to within 0.02\% at every tested severity; the 0.25 line is a modeling-scope
guard, not an accuracy boundary.}
\label{fig:fidelity}
\end{figure}
...
At the reference baseline the marginal accelerator contributes 0.502 productive
...
worth 281 purchased ones. Notably the four oversubscription curves in
...
The informal metric equals 0.73 of SEA at 2{,}048 accelerators and 0.22 at
65{,}536, understating intervention value by factors of 1.4 and 4.6 respectively.
...
substitutes as expected (mean additivity error $+6.5\%$), and a mixed network and
reliability bundle likewise ($+12.0\%$). Recovery improvements are
\emph{complements} ($-8.9\%$): faster checkpointing shortens the optimal
...
roughly 280 to 2{,}000 accelerators depending on the regime, so a single quoted
value for such an intervention would be misleading. One caveat applies to the
zero-valued rows: the model holds the parallelism plan fixed, so it prices what
a larger scale-up domain or a deeper spare pool does for \emph{this} plan. The
real value of a larger scale-up domain lies in the parallelism plans it
enables, which is outside this study's scope, so those zeros are lower bounds.
...
failure-rate by oversubscription grid. Three different interventions rank first,
and the winning mix differs between the two scales. Blank cells fall outside the
scope guard.}
\label{fig:decision}
\end{figure}

Figure~\ref{fig:decision} is the central result. Across 96 in-scope cells,
three different interventions rank first. At 16{,}384 accelerators the map
divides evenly between straggler control at lower failure rates and halving the
failure rate at higher ones; additional bandwidth never wins. At 65{,}536
accelerators reliability dominates most of the map, quadrupled bandwidth takes
the low-failure, heavily oversubscribed corner, and straggler control retains a
low-failure fringe. Faster restart, which wins nowhere outright, is still worth
hundreds to thousands of accelerators in absolute terms
(Figure~\ref{fig:sea}); ranking first and being worth funding are different
questions.
...
documented parameter ranges (Figure~\ref{fig:stability}). At 16{,}384 accelerators, cutting the blocking checkpoint cost to 5 seconds
ranks first in 45.8\% of draws, halving the failure rate in 29.1\%, straggler
control in 15.8\%, faster restart in 8.7\%, and quadrupled bandwidth in 0.6\%.
The absolute-target interventions (deploying a specific checkpoint, detection,
or restart capability) are strongest exactly where the drawn baseline is worst,
which is the regime dependence the map displays.

\section{Sensitivity and counterexamples}

We sought negative cases deliberately.

\paragraph{Where network investment is worth nothing.} With an already fast and flat network (3.2 Tb/s per accelerator, no
oversubscription), a further quadrupling of bandwidth is worth 47 accelerators,
0.3\% of the pool, while halving the failure rate is worth 954. The mirror case
also holds: with near-perfect reliability, halving the failure rate is worth 18
accelerators while doubling bandwidth is worth 255.

\paragraph{Where nothing much matters.} At 1{,}024 accelerators no
intervention is worth more than 39 accelerators, because $\mathrm{UCF}$ is
...
bandwidth to 871 and 882 accelerators respectively, because
communication becomes a larger share of a smaller step.

\paragraph{Where accelerators cannot substitute at all.} At approximately 131{,}000 accelerators under this configuration, buying more
accelerators is nearly worthless and soon harmful: productive throughput rises
1.2\% to a peak of 31{,}566 at 1.25$\times$ the baseline pool of 31{,}204, then
falls to 29{,}628 at 2$\times$. Halving the failure rate yields 38{,}640, beyond
anything the scaling curve reaches, so no purchase matches the intervention.
This case lies outside the scope guard and was therefore computed with both
implementations, which agree to 0.03\%.
...
understates value by a factor that reaches six at large scale, and it cannot
...
available at
\url{https://github.com/dimaggi-ai/network-vs-more-gpus}. The full experiment program
